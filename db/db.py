...
class MeuBanco:
    ...
    def cria_tabelas(self):
        try:
            ...
            cursor.execute(
                f"""
				CREATE TABLE IF NOT EXISTS profissoes_personagens (
					{CHAVE_ID} VARCHAR(30) PRIMARY KEY NOT NULL,
					{CHAVE_ID_PROFISSAO} VARCHAR(30) NOT NULL, 
					{CHAVE_ID_PERSONAGEM} VARCHAR(30) NOT NULL, 
					{CHAVE_EXPERIENCIA} INTEGER NOT NULL, 
					{CHAVE_PRIORIDADE} TINYINT NOT NULL,
					FOREIGN KEY ({CHAVE_ID_PROFISSAO}) REFERENCES profissoes({CHAVE_ID}),
					FOREIGN KEY ({CHAVE_ID_PERSONAGEM}) REFERENCES personagens({CHAVE_ID})
				);
				"""
            )
    ...
    def remove_tabela(self, tabela: str) -> bool:
        try:
            if self.__conexao is None:
                raise Exception("Erro ao conectar com o banco.")
