...
from utilitariosTexto import texto_eh_igual, texto1_pertence_texto2, limpa_ruido_texto
...
class Aplicacao:
	def __init__(self) -> None:
		...
		self.__personagemEmUso: Personagem | None = None
		...
		self.__trabalho_vendido: TrabalhoVendido | None = None
		__meuBanco: MeuBanco= MeuBanco()
		try:
			__meuBanco.pega_conexao()
			__meuBanco.cria_tabelas()
			self.__personagemDao: PersonagemDaoSqlite= PersonagemDaoSqlite(banco= __meuBanco)
			self.__profissaoDao: ProfissaoPersonagemDaoSqlite= ProfissaoPersonagemDaoSqlite(banco= __meuBanco)
	...
	def defineListaPersonagemMesmoEmail(self) -> list[Personagem]:
		...
		for personagem in personagens:
			if texto_eh_igual(personagem.email, self.__personagemEmUso.email):
	...
	def modificaAtributoUso(self) -> None: 
		...
		for personagem in personagens:
			if self.verificaPersonagemMesmoEmail(listaPersonagemMesmoEmail, personagem) and personagem.uso:
				...
				self.modifica_personagem(personagem)
	...
	def verificaPersonagemMesmoEmail(self, listaPersonagemMesmoEmail: list[Personagem], personagem: Personagem) -> bool:
		for personagemMesmoEmail in listaPersonagemMesmoEmail:
			if texto_eh_igual(personagem.id, personagemMesmoEmail.id) and not personagem.uso:
				personagem.alternaUso
				self.modifica_personagem(personagem)
				return False
		return True
	...
	def retornaCodigoErroReconhecido(self, textoErroEncontrado: str) -> int:
		...
		for posicaoTipoErro in range(len(CHAVE_LISTA_ERROS)):
			textoErro: str = limpa_ruido_texto(CHAVE_LISTA_ERROS[posicaoTipoErro])
	...
	def retorna_menu(self) -> int | None:
		...
		texto_menu = self.__imagem.retorna_texto_sair()
	...
	def insere_trabalho_vendido(
			self, 
			personagem: Personagem | None = None,
			modifica_servidor: bool = True
		) -> bool:
		personagem = self.__personagemEmUso if personagem is None else personagem
		if self.__trabalho_vendido and personagem and self.__vendasDao.insere_trabalho_vendido(
			personagem= personagem,
			trabalho= self.__trabalho_vendido,
			modificaServidor= modifica_servidor
		):
			self.__loggerVendaDao.info(f'({personagem.id.ljust(36)} | {self.__trabalho_vendido}) inserido no banco com sucesso!')
			return True
		self.__loggerVendaDao.error(f'Erro ao inserir ({personagem.id.ljust(36)} | {self.__trabalho_vendido}) no banco: {self.__vendasDao.pegaErro}')
		return False
	...
	def reconhece_conteudo_correspondencia(self) -> str | None:
		'''
		...
		'''
		conteudo_correspondencia: str | None = self.__imagem.retorna_texto_correspondencia_reconhecido()
		self.__logger_aplicacao.debug(mensagem= f'Conteúdo correspondencia: {conteudo_correspondencia}')
		return conteudo_correspondencia
	
	def processa_conteudo_reconhecido(self, conteudo: str | None):
		trabalho_foi_vendido: bool = texto1_pertence_texto2(texto1= 'Vendido', texto2= conteudo)
		if trabalho_foi_vendido:
			self.define_trabalho_vendido(conteudo)

	def define_trabalho_vendido(self, conteudo_correspondencia: str | None):
		'''
		...
		conteudo_formatado: str = re.sub("Item", "", conteudo_correspondencia).strip()
		conteudo_formatado: str = re.sub("vendido", "", conteudo_correspondencia).strip()
		trabalho: TrabalhoVendido = TrabalhoVendido()
		trabalho.descricao = conteudo_formatado
		trabalho.dataVenda = str(datetime.date.today())
		trabalho.setQuantidade(self.retornaQuantidadeTrabalhoVendido(conteudo_formatado))
		trabalho.idTrabalho = self.retornaChaveIdTrabalho(conteudo_formatado)
		trabalho.setValor(self.retornaValorTrabalhoVendido(conteudo_formatado))
		self.__trabalho_vendido = trabalho
	...
	def recupera_trabalhos_estoque(self, personagem: Personagem | None = None) -> list[TrabalhoEstoque]:
		'''
		...
		'''
		personagem = self.__personagemEmUso if personagem is None else personagem
		trabalhos_estoque: list[TrabalhoEstoque]= self.__estoqueDao.recupera_trabalhos_estoque(personagem= personagem)
		if trabalhos_estoque is None:
			self.__loggerEstoqueDao.error(f'Erro ao buscar trabalhos em estoque no banco: {self.__estoqueDao.pegaErro}')
			return []
		return trabalhos_estoque

	def atualiza_quantidade_trabalho_estoque(self):
		estoque: list[TrabalhoEstoque] = self.recupera_trabalhos_estoque()
		if self.__trabalho_vendido is None:
			return

		for trabalho_estoque in estoque:
			if texto_eh_igual(trabalho_estoque.idTrabalho, self.__trabalho_vendido.idTrabalho):
				trabalho_estoque.decrementa_quantidade(self.__trabalho_vendido.quantidade)
				if self.modifica_trabalho_estoque(trabalho_estoque):
					self.__loggerEstoqueDao.debug(f'Quantidade de ({trabalho_estoque}) atualizada para {trabalho_estoque.quantidade}.')
				return

		self.__loggerEstoqueDao.warning(f'({self.__trabalho_vendido}) não encontrado no estoque.')
	...
	def recupera_correspondencia(self):
		'''
			Método para verificar a existência de correspondências. 
		'''
		while self.__imagem.existe_correspondencia():
			click_especifico(cliques= 1, tecla_especifica= 'enter')
			conteudo_correspondencia: str | None = self.reconhece_conteudo_correspondencia()
			self.processa_conteudo_reconhecido(conteudo_correspondencia)
			click_especifico(cliques= 1, tecla_especifica= 'f2')
			if self.__trabalho_vendido is None:
				continue
			self.insere_trabalho_vendido()
			self.atualiza_quantidade_trabalho_estoque()
	...
	def insereListaProfissoes(self, personagem: Personagem | None = None) -> bool:
		personagem: Personagem | None = self.__personagemEmUso if personagem is None else personagem
		if self.__profissaoDao.insere_lista_profissoes(personagem= personagem):
			return True
		return False
		...
		if trabalho.eh_producao_recursos:
			if trabalhoEhProducaoLicenca(trabalhoConcluido):
				...
				if variavelExiste(trabalhoEstoque):
					if texto_eh_igual(trabalhoConcluido.tipoLicenca, CHAVE_LICENCA_APRENDIZ):
			...
			if trabalhoEhColecaoRecursosComuns(trabalhoConcluido) or trabalhoEhColecaoRecursosAvancados(trabalhoConcluido):
					...
					trabalhos = self.pegaTrabalhosBanco()
					for trabalho in trabalhos:
						condicoes = texto_eh_igual(trabalho.profissao, trabalhoConcluido.profissao) and trabalho.nivel == nivelColecao and texto_eh_igual(trabalho.raridade, CHAVE_RARIDADE_COMUM)
						if condicoes:
							trabalhoEstoque = TrabalhoEstoque()
			...
			if eh_vazia(listaTrabalhoEstoqueConcluido):
					trabalhoEstoque = TrabalhoEstoque()
	...
	def modificaQuantidadeTrabalhoEstoque(self, listaTrabalhoEstoqueConcluido: list[TrabalhoEstoque], trabalhoEstoque: TrabalhoEstoque):
		...
		for trabalhoEstoqueConcluido in listaTrabalhoEstoqueConcluido:
			if texto_eh_igual(trabalhoEstoqueConcluido.nome, trabalhoEstoque.nome):
				...
				if self.modifica_trabalho_estoque(trabalhoEstoque):
	...
	def atualizaEstoquePersonagem(self, trabalhoConcluido: TrabalhoProducao):
		...
		for trabalhoEstoqueConcluido in listaTrabalhoEstoqueConcluido:
			...
			self.modifica_trabalho_estoque(trabalhoEncontrado)
	...
	def verificaProducaoTrabalhoRaro(self, trabalhoConcluido: TrabalhoProducao) -> TrabalhoProducao | None:
		...
		if trabalhoConcluido.ehMelhorado:
			...
			trabalhos: list[Trabalho] = self.pegaTrabalhosBanco()
			for trabalho in trabalhos:
				trabalhoNecessarioEhIgualIdTrabalhoConcluido = texto_eh_igual(texto1= trabalho.trabalhoNecessario, texto2= trabalhoConcluido.idTrabalho)
	...
	def reconhece_menu_recompensa(self, codigoMenu: int) -> bool:
		'''
	...
	def recebe_todas_recompensas(self, codigoMenu: int):
		'''
		...
		verificacoes: int = 0
		codigo: int = codigoMenu
		while True:
			if verificacoes > QUANTIDADE_MAXIMA_PERSONAGENS_POSSIVEIS:
				break
			verificacoes += 1
			if self.reconhece_menu_recompensa(codigoMenu= codigo):
				if self.__imagem.retorna_existe_pixel_correspondencia():
					vai_para_menu_correspondencia()
					self.recupera_correspondencia()
					self.ofertaTrabalho()
	...
	def trataMenu(self, menu) -> None:
		...
		if ehMenuTrabalhosAtuais(menu= menu):
			estadoTrabalho: int | None = self.__imagem.retornaEstadoTrabalho()
			if estadoTrabalho == CODIGO_CONCLUIDO:
				nomeTrabalhoConcluido: str | None = self.reconhece_recupera_trabalho_concluido()
		...
		if ehMenuRecompensasDiarias(menu= menu) or ehMenuLojaMilagrosa(menu= menu):
			self.recebe_todas_recompensas(menu)
	...
	def retorna_trabalhos_raros_vendidos_ordenada(self, listaTrabalhosRarosVendidos):
		...
		for trabalhosRarosVendidos in listaTrabalhosRarosVendidos:
			if eh_vazia(listaTrabalhosRarosVendidosOrdenada):
			...
			for trabalhoRaroVendidoOrdenado in listaTrabalhosRarosVendidosOrdenada:
				if texto_eh_igual(trabalhoRaroVendidoOrdenado.nome, trabalhosRarosVendidos.nome):
	...
	def pega_trabalhos_por_profissao_raridade(self, trabalho: Trabalho) -> list[Trabalho]:
		trabalhos = self.__trabalhoDao.pegaTrabalhosPorProfissaoRaridade(trabalho)
		if trabalhos is None:
			self.__loggerTrabalhoDao.error(
				f'Erro ao buscar trabalhos por profissão e raridade ({trabalho.profissao}, {trabalho.raridade}) no banco: {self.__trabalhoDao.pegaErro}'
			)
			return []
		return trabalhos
	...
	def verifica_produtos_raros_mais_vendidos(self):
		...
		if eh_vazia(lista_trabalhos_raros_vendidos):
	...
	def mostraListaProfissoesNecessarias(self) -> None:
		if eh_vazia(self.__listaProfissoesNecessarias):
	...
	def retornaListaTrabalhosProducaoRaridadeEspecifica(self, nomeProfissao: str, raridade: str) -> list[TrabalhoProducao]:
		...
		if eh_vazia(listaTrabalhosProducaoRaridadeEspecifica): self.__logger_aplicacao.debug(f'Nem um trabalho {raridade} na lista!')
	...
	def retorna_texto_trabalho_posicao_trabalho_raro_especial(self, dicionario_trabalho: dict):
		return self.__imagem.retorna_nome_trabalho_reconhecido((dicionario_trabalho[CHAVE_POSICAO] * 72) + 289, 0)

	def retornaFrameTelaTrabalhoEspecifico(self) -> str | None:
		click_especifico(1, 'down')
		click_especifico(1, 'enter')
		nomeTrabalhoReconhecido: str = self.__imagem.retorna_nome_confirmacao_trabalho_producao_reconhecido(tipoTrabalho= 0)
		click_especifico(1, 'f1')
		click_especifico(1, 'up')
	...
	def confirma_nome_trabalho_producao(self, dicionario: dict, tipo: int):
		...
		if tipo == 0:
			...
			nomeTrabalhoReconhecido: str = self.__imagem.retorna_nome_confirmacao_trabalho_producao_reconhecido(tipoTrabalho= tipo)
		...
		if CHAVE_LISTA_TRABALHOS_PRODUCAO_PRIORIZADA in dicionario:
			...
			for trabalhoProducao in listaTrabalhoProducaoPriorizada:
				...
				if trabalhoEncontrado.eh_producao_recursos:
					nomeProducaoTrabalho: str = limpa_ruido_texto(trabalhoEncontrado.nomeProducao)
					if texto1_pertence_texto2(nomeTrabalhoReconhecido, nomeProducaoTrabalho):
						...
						return dicionario
					continue
				if texto_eh_igual(nomeTrabalhoReconhecido, nomeTrabalho):
					dicionario[CHAVE_TRABALHO_PRODUCAO_ENCONTRADO] = trabalhoProducao
				...
				if texto_eh_igual(nomeTrabalhoReconhecido, nomeProducaoTrabalho):
					dicionario[CHAVE_TRABALHO_PRODUCAO_ENCONTRADO] = trabalhoProducao
					self.__logger_trabalho_producao_dao.info(f'Trabalho confirmado: {nomeTrabalhoReconhecido.ljust(30)} | {nomeProducaoTrabalho.ljust(30)}')
					return dicionario
	...
	def padronizaTexto(self, texto: str) -> str:
		...
		textoPadronizado = limpa_ruido_texto(texto= textoPadronizado)
	...
	def reconhece_texto_trabalho_comum_melhorado(self, trabalho: dict, contadorParaBaixo: int) -> str | None:
		...
		if primeira_busca(trabalho):
			...
			return self.__imagem.retorna_nome_trabalho_reconhecido(yinicialNome, 1)
		...
		if contadorParaBaixo == 4:
			...
			return self.__imagem.retorna_nome_trabalho_reconhecido(yinicialNome, 1)
		if contadorParaBaixo > 4:
			return self.__imagem.retorna_nome_trabalho_reconhecido(530, 1)
	...
	def define_dicionario_trabalho_comum_melhorado(self, dicionario_trabalho: dict) -> dict:
		...
		while not chave_dicionario_trabalho_desejado_existe(dicionario_trabalho):
			...
			if nome_reconhecido_nao_esta_vazio_e_nao_eh_fim_lista:
				if texto_eh_igual(texto1= nome_reconhecido_aux, texto2= nome_trabalho_reconhecido):
	...
	def defineTrabalhoRecurso(self, trabalho: Trabalho) -> TrabalhoRecurso:
		...
		if texto_eh_igual(texto1= trabalho.profissao, texto2= CHAVE_PROFISSAO_ARMA_DE_LONGO_ALCANCE):
			recursoTerciario = 1
	...
	def atualizaResursosEstoqueTrabalhoMelhoradoRaroProduzindo(self, trabalho: Trabalho) -> None:
		...
		for idTrabalhoNecessario in listaIdsTrabalhosNecessarios:
			...
			if trabalhoEncontrado.idTrabalho == idTrabalhoNecessario:
				...
				if self.modifica_trabalho_estoque(trabalhoEncontrado):
	...
	def atualizaRecursosEstoqueTrabalhoRecursoProduzindo(self, trabalhoProducao: TrabalhoProducao) -> None:
		...
		for trabalhoEstoque in self.recupera_trabalhos_estoque():
			for recursoBuscado in listaNomeRecursoBuscado:
				if texto_eh_igual(trabalhoEstoque.nome, recursoBuscado[0]):
			...
			if texto_eh_igual(trabalhoEstoque.nome, trabalhoProducao.tipoLicenca):
	...
	def atualizaRecursosEstoqueTrabalhoComumProduzindo(self, trabalhoProducao: TrabalhoProducao):
		...
		for trabalhoEstoque in self.recupera_trabalhos_estoque():
			if texto_eh_igual(texto1= trabalhoEstoque.nome, texto2= trabalhoRecurso.primario):
				...
				continue
			if texto_eh_igual(texto1= trabalhoEstoque.nome, texto2= trabalhoRecurso.secundario):
				trabalhoEstoque.setQuantidade(trabalhoEstoque.quantidade - trabalhoRecurso.pegaQuantidadeSecundario)
				if self.modifica_trabalho_estoque(trabalhoEstoque):
					self.__logger_aplicacao.debug(f'Quantidade do trabalho ({trabalhoEstoque}) atualizada.')
				continue
			if texto_eh_igual(texto1= trabalhoEstoque.nome, texto2= trabalhoRecurso.terciario):
				trabalhoEstoque.setQuantidade(trabalhoEstoque.quantidade - trabalhoRecurso.pegaQuantidadeTerciario)
				if self.modifica_trabalho_estoque(trabalhoEstoque):
					self.__logger_aplicacao.debug(f'Quantidade do trabalho ({trabalhoEstoque}) atualizada.')
				continue
			if texto_eh_igual(texto1= trabalhoEstoque.nome, texto2=trabalhoProducao.tipoLicenca):
				trabalhoEstoque.setQuantidade(trabalhoEstoque.quantidade - 1)
				if self.modifica_trabalho_estoque(trabalhoEstoque):
					self.__logger_aplicacao.debug(f'Quantidade do trabalho ({trabalhoEstoque}) atualizada.')

	def trata_menu_licenca(self, trabalho_encontrado: TrabalhoProducao) -> TrabalhoProducao | None:
		self.__logger_aplicacao.debug(f"Buscando: {trabalho_encontrado.tipoLicenca}")
		texto_reconhecido: str | None = self.__imagem.retorna_texto_licenca_reconhecida()
		if texto_reconhecido is None:
			self.__logger_trabalho_producao_dao.warning(f'{trabalho_encontrado.tipoLicenca} não encontrado')
			self.__confirmacao = False
			return None
		self.__logger_aplicacao.debug(f'Licença reconhecida: {texto_reconhecido}')
		if texto1_pertence_texto2(texto1= 'Licença de Artesanato', texto2= texto_reconhecido):
			primeira_busca: bool = True
			licencas_reconhecidas: list[str] = []
			while not texto1_pertence_texto2(texto1= texto_reconhecido, texto2= trabalho_encontrado.tipoLicenca):
				primeira_busca = False
				licencas_reconhecidas.append(texto_reconhecido)
				click_especifico(cliques= 1, tecla_especifica= "right")
				texto_reconhecido: str | None = self.__imagem.retorna_texto_licenca_reconhecida()
				if texto_reconhecido is None:
					verificacaoErro: VerificacaoErro = VerificacaoErro(texto_erro_encontrado= self.__imagem.retorna_texto_menu_reconhecido)
					...
					if verificacaoErro.eh_erro_outra_conexao:
						self.__unicaConexao = False
					self.__logger_aplicacao.error(f'Erro encontrado ao buscar licença necessária')
					self.__confirmacao = False
					return None
				self.__logger_aplicacao.debug(f'Licença reconhecida: {texto_reconhecido}.')
				if texto_eh_igual(texto1= texto_reconhecido, texto2= 'nenhumitem'):
					if trabalho_encontrado.tipoLicenca == CHAVE_LICENCA_NOVATO:
						if texto_eh_igual(texto1= licencas_reconhecidas[-1], texto2= 'nenhumitem'):
							continue
						self.__logger_aplicacao.warning(f'{CHAVE_LICENCA_NOVATO} não encontrado')
						click_especifico(cliques= 3, tecla_especifica= 'f1')
						preciona_tecla(cliques= 10, teclaEspecifica= 'up')
						click_especifico(cliques= 1, tecla_especifica= 'left')
						self.__confirmacao = False
						return None
					self.__logger_aplicacao.warning(f'{trabalho_encontrado.tipoLicenca} não encontrado')
					self.__logger_aplicacao.info(f'{trabalho_encontrado.tipoLicenca} modificado para ({CHAVE_LICENCA_NOVATO})')
					trabalho_encontrado.tipoLicenca = CHAVE_LICENCA_NOVATO
					continue
				if len(licencas_reconhecidas) > 20:
					if trabalho_encontrado.tipoLicenca == CHAVE_LICENCA_NOVATO:
						self.__logger_aplicacao.warning(f'{CHAVE_LICENCA_NOVATO} não encontrado')
						click_especifico(cliques= 3, tecla_especifica= 'f1')
						preciona_tecla(cliques= 10, teclaEspecifica= 'up')
					...
					self.__logger_aplicacao.info(f'{trabalho_encontrado.tipoLicenca} modificado para ({CHAVE_LICENCA_NOVATO})')
					trabalho_encontrado.tipoLicenca = CHAVE_LICENCA_NOVATO
					licencas_reconhecidas = []
			trabalho_encontrado.estado = CODIGO_PRODUZINDO
			click_especifico(cliques= 1, tecla_especifica= "f1") if primeira_busca else click_especifico(cliques= 1, tecla_especifica= "f2")
			return trabalho_encontrado
		self.__logger_aplicacao.warning(f'{trabalho_encontrado.tipoLicenca} não encontrado')
	...
	def inicia_processo_de_producao(self, dicionario_trabalho: dict) -> dict:
		primeira_busca: bool = True
		trabalho_encontrado: TrabalhoProducao = dicionario_trabalho[CHAVE_TRABALHO_PRODUCAO_ENCONTRADO]
		while True:
			self.trata_erros_processo_producao(trabalho= trabalho_encontrado)
			if not self.__confirmacao or self.verificar_novamente or dicionario_trabalho[CHAVE_TRABALHO_PRODUCAO_ENCONTRADO] is None:
				break
			menu: int = self.retorna_menu()
			if ehMenuTrabalhosAtuais(menu= menu): 
				self.trataMenuTrabalhosAtuais(trabalho= trabalho_encontrado)
				return dicionario_trabalho
			if ehMenuTrabalhoEspecifico(menu= menu):
				self.trataMenuTrabalhoEspecifico(primeiraBusca= primeira_busca)
				primeira_busca = False
				continue
			if ehMenuLicenca(menu= menu):
				dicionario_trabalho[CHAVE_TRABALHO_PRODUCAO_ENCONTRADO] = self.trata_menu_licenca(trabalho_encontrado)
				continue
	...
	def retorna_trabalho_producao_concluido(self, nome_trabalho_reconhecido: str) -> TrabalhoProducao | None:
		...
		if eh_vazia(possiveis_trabalhos_producao):
		...
		for provavel_trabalho_producao in possiveis_trabalhos_producao:
			for trabalho_produzindo_encontrado in trabalhos_producao_encontrados:
				if texto_eh_igual(trabalho_produzindo_encontrado.nome, provavel_trabalho_producao.nome):
	...
	def mostra_lista_trabalhos_producao_produzindo(self, trabalhos_producao: list[TrabalhoProducao]):
		if eh_vazia(trabalhos_producao):
	...
	def verificaProfissaoNecessaria(self, dicionarioTrabalho: dict, profissao: Profissao):
		while self.__confirmacao:
			...
			if chave_dicionario_trabalho_desejado_existe(dicionarioTrabalho):
				dicionarioTrabalho = self.inicia_processo_de_producao(dicionarioTrabalho)
			...
			if self.__confirmacao:
				if self.__unicaConexao and self.__espacoBolsa:
					if self.__imagem.retornaEstadoTrabalho() == CODIGO_CONCLUIDO:
						nomeTrabalhoReconhecido: str | None = self.reconhece_recupera_trabalho_concluido()
	...
	def retorna_lista_de_listas_trabalhos_producao(self, nome_profissao: str) -> list[list[TrabalhoProducao]]:
		...
		for raridade in raridades:
			...
			if eh_vazia(listaTrabalhosProducao):
				continue
	...
	def retiraPersonagemJaVerificadoListaAtivo(self):
		...
		if not eh_vazia(self.__listaPersonagemAtivo):
			...
			for personagemAtivo in self.__listaPersonagemAtivo:
				for personagemRemovido in self.__listaPersonagemJaVerificado:
					if texto_eh_igual(personagemAtivo.nome, personagemRemovido.nome):
						break
				else:
	...
	def pega_trabalhos_por_profissao_raridade_nivel(self, trabalho: Trabalho) -> list[Trabalho]:
		trabalhos_filtrados: list[Trabalho] | None = self.__trabalhoDao.pega_trabalhos_por_profissao_raridade_nivel(trabalho)
		if trabalhos_filtrados is None:
	...
	def define_trabalho_producao_recursos_profissao_priorizada(self, trabalho: Trabalho):
		...
		if eh_vazia(lista= trabalhosProducaoEncontrados):
	...
	def defineTrabalhoProducaoRecurso(self, trabalho: Trabalho) -> Trabalho | None:
		'''
			...
			Returns:
				trabalhoEncontrado (Trabalho): Objeto da classe Trabalho com atributos do trabalho para produção de recursos encontrado.
		'''
		trabalhosProducaoRecursosEncontrados: list[Trabalho] = self.pega_trabalhos_por_profissao_raridade_nivel(trabalho= trabalho)
	...
	def define_trabalho_comum_profissao_priorizada(self):
		...
		if eh_vazia(profissoes_priorizadas):
		...
		for profissao_priorizada in profissoes_priorizadas:
			...
			for trabalho in trabalhos_comuns_profissao_nivel_expecifico:
				self.__logger_aplicacao.debug(mensagem= f'({trabalho.id.ljust(40)} | {trabalho}) encontrado')
			if eh_vazia(trabalhos_comuns_profissao_nivel_expecifico):
				continue
			while True:
	...
	def define_trabalho_melhorado_profissao_priorizada(self, profissao_priorizada: Profissao, nivel_profissao: int):
		...
		if profissao_priorizada.eh_nivel_producao_melhorada:
			...
			for trabalho in trabalhos_raros_profissao_nivel_expecifico:
				self.__logger_aplicacao.debug(mensagem= f'({trabalho.id.ljust(40)} | {trabalho}) encontrado')
	...
	def define_trabalhos_por_profissao_nivel_raridade(self, trabalho_buscado: Trabalho):
		...
		trabalhos_encontrados: list[Trabalho] = self.pega_trabalhos_por_profissao_raridade_nivel(trabalho_buscado)
		if eh_vazia(trabalhos_encontrados):
	...
	def verificaListaIdsTrabalhosMelhoradosNecessarios(self, trabalho: Trabalho):
		...
		if trabalhosMelhoradosNecessarios is None or eh_vazia(trabalhosMelhoradosNecessarios):
	...
	def possuiTrabalhosNecessariosSuficientes(self, idTrabalhoMelhorado: str) -> bool:
		...
		if trabalhosComunsNecessarios is None or eh_vazia(trabalhosComunsNecessarios):
	...
	def trabalho_esta_na_lista_producao(self, id: str):
		...
		for trabalhoProducao in trabalhosParaProduzirProduzindo:
			if texto_eh_igual(trabalhoProducao.idTrabalho, id):
	...
	def listaPersonagemJaVerificadoEPersonagemAnteriorEAtualMesmoEmail(self) -> bool:
		...
		'''
		if eh_vazia(self.__listaPersonagemJaVerificado) or eh_vazia(self.__listaPersonagemAtivo):
			return False
		emailAnterior: str= self.__listaPersonagemJaVerificado[-1].email
		emailProximo: str= self.__listaPersonagemAtivo[0].email
		emailDoUltimoPersonagemEhIgualAoEmailPrimeiroPersonagemDaListaDeAtivos = texto_eh_igual(texto1= emailAnterior, texto2= emailProximo)
	...
	def listaPersonagensAtivosEstaVazia(self) -> bool:
		if eh_vazia(self.__listaPersonagemAtivo):
	...
	def insere_trabalho(self, trabalho: Trabalho, modifica_servidor: bool = True) -> bool:
		if self.__trabalhoDao.insere_trabalho(trabalho, modifica_servidor):
			self.__loggerTrabalhoDao.info(
				f'({trabalho.id.ljust(36)} | {trabalho}) inserido no banco com sucesso!'
			)
			return True

		self.__loggerTrabalhoDao.error(
			f'Erro ao inserir ({trabalho.id.ljust(36)} | {trabalho}) no banco: {self.__trabalhoDao.pegaErro}'
		)
		return False
		...
		if self.__repositorioTrabalho.estaPronto:
			...
			for trabalho in trabalhos:
				...
				if trabalhoEncontrado.id == trabalho[CHAVE_ID]:
					...
					continue
				trabalhoEncontrado.dicionarioParaObjeto(trabalho)
				self.insere_trabalho(trabalho= trabalhoEncontrado, modifica_servidor= False)
			...
			for personagem in personagens:
				...
				if self.__repositorioUsuario.verificaIdPersonagem(id= personagem[CHAVE_ID]):
					...
					if personagemEncontrado.id == personagem[CHAVE_ID]:
						...
						self.modifica_personagem(personagem= personagemEncontrado, modificaServidor= False)
						continue
					personagemEncontrado.dicionarioParaObjeto(personagem)
				...
				if self.__repositorioUsuario.verificaIdPersonagem(id= personagem.id):
					...
					if len(dicionarioTrabalho) > 1:
						if trabalhoEncontrado.id == dicionarioTrabalho[CHAVE_ID]:
							...
							self.modifica_trabalho_estoque(trabalho= trabalhoEncontrado, personagem= personagem, modificaServidor= False)
							continue
						trabalhoEncontrado.dicionarioParaObjeto(dicionarioTrabalho)
				...
				if self.__repositorioUsuario.verificaIdPersonagem(id= personagem.id):
					...
					if len(dicionarioTrabalho) > 1:
						if trabalhoEncontrado.id == dicionarioTrabalho[CHAVE_ID]:
							...
							continue
						trabalhoEncontrado.dicionarioParaObjeto(dicionarioTrabalho)
						self.insere_trabalho_vendido(trabalho= trabalhoEncontrado, personagem= personagem, modifica_servidor= False)
						continue
					trabalhoEncontrado.id = dicionarioTrabalho[CHAVE_ID]
	...
	def modifica_personagem(self, personagem: Personagem = None, modificaServidor: bool = True) -> bool:
		personagem = self.__personagemEmUso if personagem is None else personagem
	...
	def modifica_trabalho_estoque(self, trabalho: TrabalhoEstoque, personagem: Personagem = None, modificaServidor: bool = True) -> bool:
		personagem = self.__personagemEmUso if personagem is None else personagem
	...
	def insereProfissao(self, profissao: Profissao, personagem: Personagem = None, modificaServidor: bool = True) -> bool:
		personagem = self.__personagemEmUso if personagem is None else personagem
		if self.__profissaoDao.insere_profissao(personagem= personagem, profissao= profissao, modificaServidor= modificaServidor):
	...
	def sincronizaListaProfissoes(self):
		...
		for personagem in personagens:
			...
			if self.__profissaoDao.sincroniza_profissoes_por_id(personagem= personagem):
