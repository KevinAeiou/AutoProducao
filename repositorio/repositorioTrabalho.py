...
class RepositorioTrabalho(Stream):
    def __init__(self) -> None:
        ...
        self.__logger: MeuLogger= MeuLogger(nome= CHAVE_REPOSITORIO_TRABALHO, arquivoLogger= f'{CHAVE_REPOSITORIO_TRABALHO}.log')
    ...
    def pegaTodosTrabalhos(self):
        ...
        try:
            ...
            self.__logger.debug(menssagem= f'Trabalhos recuperados com sucesso!')
            return trabalhos
        except Exception as e:
            self.__erro = str(e)
            self.__logger.error(menssagem= f'Erro ao pegar trabalhos: {e}')
        return None
    ...
    def insereTrabalho(self, trabalho: Trabalho):
        try:
            ...
            self.__erro = str(e)
            self.__logger.error(menssagem= f'Erro ao inserir trabalho: {e}')
        return False
    ...
    def modificaTrabalho(self, trabalho: Trabalho):
        try:
            ...
            self.__erro = str(e)
            self.__logger.error(menssagem= f'Erro ao modificar trabalho: {e}')
        return False
                ...
                for idPersonagem in todasVendas:
                    for idVenda in todasVendas[idPersonagem]:
                        if todasVendas[idPersonagem][idVenda][CHAVE_ID_TRABALHO] == trabalho.id:
                            ...
                            self.__logger.debug(menssagem= f'Referência de ({trabalho}) removido da lista de vendas com sucesso!')
                ...
                for idPersonagem in estoques:
                    for idEstoque in estoques[idPersonagem]:
                        if estoques[idPersonagem][idEstoque][CHAVE_ID_TRABALHO] == trabalho.id:
                            ...
                            self.__logger.debug(menssagem= f'Referência de ({trabalho}) removido da lista de estoque com sucesso!')
                ...
                for idPersonagem in producoes:
                    for idProducao in producoes[idPersonagem]:
                        if producoes[idPersonagem][idProducao][CHAVE_ID_TRABALHO] == trabalho.id:
                            ...
                            self.__logger.debug(menssagem= f'Referência de ({trabalho}) removido da lista de produções com sucesso!')
            return True
        except Exception as e:
            self.__erro = str(e)
            self.__logger.error(menssagem= f'Erro ao remover trabalho: {e}')
        return False
